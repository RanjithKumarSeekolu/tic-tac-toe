...
export class TicTacToe{
    ...
    markPosition=(pos:number,currPlayer:Player):boolean=>{
        const {gridSize,board,rl}=this;
        const row:number=Math.floor((pos-1)/gridSize);
        const col:number=(pos-1)%gridSize;
        if(row && col && currPlayer && row>=0 && row<gridSize && col>=0 && col<gridSize && board[row][col]!=='X' && board[row][col]!=='O'){
    ...
    askMove=()=>{
        const {rl}=this;
        rl.question(`Player ${this.currPlayer}'s turn: Enter cell number: `, (input:string) => {
            ...
            this.markPosition(Number(input),currPlayer);
        });
    }
