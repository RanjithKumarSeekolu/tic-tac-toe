...
describe("Test suite for creating instance", function () {
  it("fail: should create board with size:3, winLength:4", function () {
    expect(() => new TicTacToe(4, 8)).toThrow(
      new Error("Win length should be less than gid size")
    );
  });
  ...
  it("fail: should create board with size:1, winLength:1", function () {
    expect(() => new TicTacToe(1, 1)).toThrow(
      new Error("Grid size shouldn't be less than 3")
    );
  });
  it("fail: should create board with size:2, winLength:3", function () {
    expect(() => new TicTacToe(2, 3)).toThrow(
      new Error("Grid size shouldn't be less than 3")
    );
  });
});
...
describe("Test suite for 5X5 grid & winLength:4", function () {
  beforeEach(function () {
    this.board = new TicTacToe(5, 4);
  });
